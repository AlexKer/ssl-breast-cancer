...
def save_as_dicom_and_npy(level_three_patient_set : set, root_dir: str, save_path : str):
    ...
    for patient_folder in tqdm(sorted(glob.glob(root_dir + "/*"))): 
        ...
        def extract_date_from_timepoint(timepoint):
            ...
            date_str = timepoint.split("\\")[-1].split("-")[0:3]  # Extract 'MM-DD-YYYY'
...
def main():
    ...
    '''
    ispy1_dir = r"Path\to\I-SPY-1"
    level_three_clinical_data = r"Path\to\I-SPY-1\I-SPY-1 Clinical Data.xlsx"
    ...
    create_json(
        saved_patients,
        patient_data,
        output_path = r"Path\to\save\the\json\file.json"
    )
